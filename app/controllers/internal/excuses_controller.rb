module Internal
  class ExcusesController < InternalController
    ...
    def index
      @excuses = if current_user.admin? || current_user.officer?
                   Excuse
    ...
    def create
      ...
      if @excuse.recurring?
        ...
        if @excuse.save
          ...
          redirect_to internal_excuses_path, notice: "Recurring excuse submitted! Applied to #{matching_events.size} event(s)."
        else
        ...
        if @excuse.save
          ...
          redirect_to internal_excuses_path, notice: 'Excuse submitted!'
        else
          log_create_failure(@excuse)
          render :new
        end
    ...
    def update
      if current_user.super_admin?
        ...
        redirect_to internal_excuse_path(@excuse), alert: 'Invalid status' and return unless %w[approved denied].include?(final_status)
        ...
        log_action('excuse_finalized', 
                   { excuse_id: @excuse.id, final_status: final_status, admin_id: current_user.id })
        redirect_to internal_excuse_path(@excuse), notice: "Excuse #{final_status}."

      elsif current_user.officer?
        # Officer makes a provisional decision; does NOT change final status
        prov_status = params[:status]
        redirect_to internal_excuse_path(@excuse), alert: 'Invalid provisional status' and return unless %w[approved denied].include?(prov_status)
        ...
        redirect_to internal_excuse_path(@excuse), notice: "Officer decision recorded — awaiting admin finalization."
      else
        log_authorization_failure('review_excuse', { excuse_id: @excuse.id })
        redirect_to internal_excuses_path, alert: 'Not authorized.'
      end
    end
    ...
    def review
      unless current_user.admin? || current_user.officer?
        ...
        redirect_to internal_excuses_path, alert: 'Not authorized.' and return
      end

      unless %w[approved denied].include?(@excuse.status)
        redirect_to internal_excuse_path(@excuse), 
                    alert: 'Only processed (approved/denied) excuses may be reviewed.' and return
      end

      redirect_to internal_excuse_path(@excuse), notice: 'You have already reviewed this excuse.' and return if @excuse.reviewers.exists?(id: current_user.id)
    ...
    # POST /internal/excuses/:id/cancel_recurring
    ...
    def cancel_recurring
      ...
      unless @excuse.recurring?
        redirect_to internal_excuse_path(@excuse), alert: 'This is not a recurring excuse.' and return
      end
      ...
      redirect_to internal_excuse_path(@excuse), notice: "Cancelled #{removed_count} future event(s) from this recurring excuse."
    end

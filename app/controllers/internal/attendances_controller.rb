...
module Internal
  class AttendancesController < InternalController
    include Loggable
    ...
    def new
      ...
      @users.each do |user|
        next if @attendances[user.id]

        @attendances[user.id] = if @excused_user_ids.include?(user.id)
                                  @event.attendances.build(user: user, status: 'excused')
                                else
                                  @event.attendances.build(user: user, status: 'absent')
                                end
      end
    end
    ...
    def create
      ...
      redirect_to internal_event_path(@event)
    ...
    def self_checkin_form
      ...
      return unless existing_attendance

...
class UsersController < ApplicationController
  ...
  def update
    if @user.update(user_params)
      ...
      redirect_to @user, notice: 'User information updated successfully.'
    else
  ...
  def promote_to_officer
    ...
    redirect_to users_path, notice: "#{@user.full_name} has been promoted to officer."
  ...
  def promote_to_super_admin
    ...
    redirect_to users_path, notice: "#{@user.full_name} has been promoted to super admin."
  rescue StandardError => e
    log_action('user_promotion_failed', { 
      target_user_id: @user.id, 
      error: e.message 
  ...
  def demote_to_user
    ...
    redirect_to users_path, notice: "#{@user.full_name} has been demoted to user."
  rescue StandardError => e
  ...
  def demote_to_officer
    ...
    redirect_to users_path, notice: "#{@user.full_name} has been demoted to officer."
  rescue StandardError => e
    log_action('user_demotion_failed', { 
      target_user_id: @user.id, 
      error: e.message 
  ...
  def destroy
    ...
    redirect_to users_path, notice: "#{user_name} has been permanently deleted from the system."
  rescue StandardError => e
    log_action('user_deletion_failed', { 
      target_user_id: @user.id, 
      error: e.message 
    })
    redirect_to users_path, alert: "Failed to delete user: #{e.message}"
  end
